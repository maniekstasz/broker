...
app.controller('ListCtrl', [
		...
		function($scope, RestService, list, $location,
				EventPublisher, $routeParams) {
			init();
		...
		function($rootScope, $scope, $http, Base64, $http,storage) {
			...
			$scope.submit = function() {
				...
				$http.get(domain + "/login", {
					...
					$http.get(domain + "/users/" + loggedUser.id +"/bidders").success(function(bidders){
						if(bidders.content.length > 0){
							$rootScope.loggedUser.bidderId = bidders.content[0].id;
						}
...
app.controller("BidderRegCtrl",['$scope',
                    		'RestService',
                    		'$location',
            				...
            				$scope.webHooks.invoice = {};
                    			$scope.submit = function() {
                    		    	...
                    		    	if($scope.webHooks.reservation.uri)
                    		    		$scope.resource.content.webHooks.push({uri: $scope.webHooks.reservation.uri, type: "RESERVATION_REQUEST"});
                    		    	if($scope.webHooks.invoice.uri)
                    		    		$scope.resource.content.webHooks.push({uri: $scope.webHooks.invoice.uri, type: "INVOICE_SEND"});
                    		    	RestService.save(resourcesData["bidder"].collectionUrl, $scope.resource, function(bidder){
                    		    		$location.path("/");
                    		    	});
                    			};
								...
								$scope.update = false;
                     			...
                     			$scope.submit = function() {
                     		    	...
                     		    	RestService.save(resourcesData["offer"].collectionUrl, $scope.resource, function(offer){
                     		    		$location.path("/bidders/"+$scope.loggedUser.bidderId + "/offers");
                     		    	});
                     			};
...
app.controller("PremiseReservationUpdateCtrl",['$scope',
                            		...
                            		'$location',
                            		'resource','$filter',
                            		function($scope, RestService, $location, resource,$filter) {
    								$scope.resource = resource;
    								$scope.update = true;
                            			$scope.submit = function(){
                             				$scope.resource.content.reservedFrom = $filter('date')($scope.resource.content.reservedFrom, "yyyy-MM-dd'T'12:00:00.000'Z");
...
app.controller('OfferCtrl', ['$scope', 'RestService', '$location','$controller', function($scope, RestService, $location,$controller){
	$controller("ElemCtrl",{$scope:$scope, RestService:RestService, $location:$location});
	$scope.addToFavourite = function(){
		var res = {};
		res.content = {};
		res.content.offer = $scope.resource.links.self;
		RestService.save(domain + "/favouriteOffers", res, function(resource){
			$scope.changeLocation(domain + "/users/" +  $scope.loggedUser.id + "/favouriteOffers");
		});
	}
}]);
